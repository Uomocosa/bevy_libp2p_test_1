...
pub fn detect_click(
    ...
    mut click_targets: Query<(Entity, &Owner, &mut ClickCounter)>,
    ...
    for (entity, owner, mut counter) in &mut click_targets {
        if let Ok(transform) = click_targets.get_component::<GlobalTransform>(entity) {
